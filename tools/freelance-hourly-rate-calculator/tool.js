(function(){
  ...
  var currentSlug = "freelance-hourly-rate-calculator";
  ...
  var fields = ["salary","expenses","tax","hoursWeek","weeksYear"];
  var dateFields = [];
  ...
  function compute(v){
      const s=parseFloat(v.salary||'0'), e=parseFloat(v.expenses||'0');
      const t=parseFloat(v.tax||'0')/100;
      const hw=parseFloat(v.hoursWeek||'0'), wy=parseFloat(v.weeksYear||'0');
      const gross = (s+e)/(1-t);
      const hours = hw*wy;
      const rate = hours? gross/hours : 0;
      return 'Gross revenue needed: '+gross.toFixed(0)+'\nBillable hours/year: '+hours+'\nHourly rate: '+rate.toFixed(2);
    }
