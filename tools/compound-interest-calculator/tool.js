(function(){
  ...
  var currentSlug = "compound-interest-calculator";
  ...
  var fields = ["principal","rate","years","freq"];
  var dateFields = [];
  ...
  function compute(v){
      const P=parseFloat(v.principal||'0'), r=parseFloat(v.rate||'0')/100, y=parseFloat(v.years||'0');
      const map={'Annually':1,'Quarterly':4,'Monthly':12,'Daily':365}; const n=map[v.freq||'Annually'];
      const A=P*Math.pow(1+r/n, n*y);
      const interest=A-P;
      return 'Future value: '+A.toFixed(2)+'\nInterest earned: '+interest.toFixed(2)+'\nEffective annual return: '+((Math.pow(1+r/n,n)-1)*100).toFixed(2)+'%';
    }
