(function(){
  ...
  var currentSlug = "customer-acquisition-cost-calculator";
  ...
  var fields = ["spend","customers","arpu","margin"];
  var dateFields = [];
  ...
  function compute(v){
      const s=parseFloat(v.spend||'0'), c=parseFloat(v.customers||'0');
      const a=parseFloat(v.arpu||'0'), m=parseFloat(v.margin||'0')/100;
      if(c<=0) return 'Enter number of customers acquired';
      const cac=s/c;
      const monthlyProfit=a*m;
      const payback=monthlyProfit>0?cac/monthlyProfit:0;
      return 'CAC: '+cac.toFixed(2)+'\nPayback period: '+payback.toFixed(1)+' months';
    }
