(function(){
  ...
  var currentSlug = "quote-to-invoice-converter";
  ...
  var fields = ["customer","invoiceNo","amount","tax"];
  var dateFields = [];
  ...
  function compute(v){
      const c=v.customer||'', n=v.invoiceNo||'', a=parseFloat(v.amount||'0');
      const tr=parseFloat(v.tax||'0')/100;
      const tax=a*tr, total=a+tax;
      return 'Bill to: '+c+'\nInvoice #: '+n+'\nDate: '+new Date().toISOString().slice(0,10)+'\n\nSubtotal: '+a.toFixed(2)+'\nTax ('+(tr*100).toFixed(0)+'%): '+tax.toFixed(2)+'\nTotal: '+total.toFixed(2);
    }
