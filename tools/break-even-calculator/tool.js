(function(){
  ...
  var currentSlug = "break-even-calculator";
  ...
  function compute(v){
      const F=parseFloat(v.fixed||'0'), Vc=parseFloat(v.vc||'0'), P=parseFloat(v.price||'0');
      const cm = P - Vc;
      if(cm<=0) return 'Break-even impossible — price must be higher than variable cost.';
      const units = F/cm;
      const rev = units*P;
      return 'Units to break even: '+Math.ceil(units)+'\nRevenue to break even: '+rev.toFixed(2)+'\nContribution margin per unit: '+cm.toFixed(2);
    }
