(function(){
  ...
  var currentSlug = "cash-flow-runway-calculator";
  ...
  var fields = ["cash","revenue","expenses"];
  var dateFields = [];
  ...
  function compute(v){
      const c=parseFloat(v.cash||'0'), r=parseFloat(v.revenue||'0'), e=parseFloat(v.expenses||'0');
      const burn = e - r;
      if(burn<=0) return 'You are cash-flow positive. No runway calculation needed.';
      const months = c/burn;
      const zero = new Date(Date.now()+months*30*86400000);
      return 'Monthly burn: '+burn.toFixed(0)+'\nRunway: '+months.toFixed(1)+' months\nOut of cash on: '+zero.toISOString().slice(0,10);
    }
