(function(){
  ...
  var currentSlug = "markup-calculator";
  ...
  function compute(v){
      const c=parseFloat(v.cost||'0'), m=parseFloat(v.markup||'0')/100;
      const price=c*(1+m); const profit=price-c; const margin=price>0?(profit/price)*100:0;
      return 'Selling price: '+price.toFixed(2)+'\nProfit per unit: '+profit.toFixed(2)+'\nResulting margin: '+margin.toFixed(1)+'%';
    }
