(function(){
  ...
  var currentSlug = "churn-rate-calculator";
  ...
  function compute(v){
      const s=parseFloat(v.start||'0'), l=parseFloat(v.lost||'0'), a=parseFloat(v.arpu||'0');
      const churn=s>0?(l/s)*100:0; const retention=100-churn;
      const annualChurn=(1-Math.pow(1-churn/100,12))*100;
      const ltv=churn>0?(a/(churn/100)):0;
      return 'Monthly churn: '+churn.toFixed(2)+'%\nAnnual churn (compounded): '+annualChurn.toFixed(1)+'%\nMonthly retention: '+retention.toFixed(2)+'%'+(a>0?'\nAvg customer lifetime value: '+ltv.toFixed(0):'');
    }
