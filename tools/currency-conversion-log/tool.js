(function(){
  ...
  var currentSlug = "currency-conversion-log";
  ...
  var fields = ["date","amount","fromCur","toCur","rate"];
  var dateFields = ["date"];
  ...
  function compute(v){
      const d=v.date||new Date().toISOString().slice(0,10);
      const a=parseFloat(v.amount||'0'), r=parseFloat(v.rate||'0');
      const converted=a*r;
      return 'Date: '+d+'\nFrom: '+a+' '+(v.fromCur||'')+'\nRate: '+r+'\nTo: '+converted.toFixed(2)+' '+(v.toCur||'');
    }
