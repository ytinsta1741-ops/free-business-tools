(function(){
  ...
  var currentSlug = "customer-lifetime-value-calculator";
  ...
  function compute(v){
      const a=parseFloat(v.arpu||'0'), m=parseFloat(v.margin||'0')/100, c=parseFloat(v.churn||'0')/100;
      if(c<=0) return 'Enter a churn rate above 0 to calculate LTV';
      const ltv=(a*m)/c;
      const lifetimeMonths=1/c;
      return 'LTV: '+ltv.toFixed(0)+'\nAvg customer lifetime: '+lifetimeMonths.toFixed(1)+' months';
    }
