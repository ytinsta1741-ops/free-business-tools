(function(){
  ...
  var currentSlug = "arr-calculator";
  ...
  function compute(v){
      const m=parseFloat(v.mrr||'0'), g=parseFloat(v.growth||'0')/100;
      const arr=m*12; const q=m*3;
      let next12=m; let projArr=0;
      for(let i=0;i<12;i++){ next12=next12*(1+g); projArr+=next12; }
      return 'Current ARR: '+arr.toFixed(0)+'\nQuarterly revenue: '+q.toFixed(0)+'\nProjected ARR in 12 months at '+(g*100).toFixed(1)+'%/mo growth: '+projArr.toFixed(0);
    }
