(function(){
  ...
  var currentSlug = "inflation-calculator";
  ...
  function compute(v){
      const a=parseFloat(v.amount||'0'), r=parseFloat(v.rate||'0')/100, y=parseFloat(v.years||'0');
      const futureValue=a*Math.pow(1+r, y);
      const realValue=a/Math.pow(1+r, y);
      return 'Nominal value needed in '+y+' years: '+futureValue.toFixed(0)+'\nReal value of today\'s '+a.toFixed(0)+' after '+y+' years: '+realValue.toFixed(0)+'\nCumulative inflation: '+((Math.pow(1+r,y)-1)*100).toFixed(1)+'%';
    }
