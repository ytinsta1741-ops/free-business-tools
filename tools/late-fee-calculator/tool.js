(function(){
  ...
  var currentSlug = "late-fee-calculator";
  ...
  var fields = ["amount","days","rate","flat"];
  var dateFields = [];
  ...
  function compute(v){
      const a=parseFloat(v.amount||'0'), d=parseFloat(v.days||'0');
      const r=parseFloat(v.rate||'0')/100, flat=parseFloat(v.flat||'0');
      const daily=r/365;
      const interest=a*daily*d;
      const total=a+interest+flat;
      return 'Original: '+a.toFixed(2)+'\nInterest ('+d+' days): '+interest.toFixed(2)+'\nFlat fee: '+flat.toFixed(2)+'\nTotal due: '+total.toFixed(2);
    }
